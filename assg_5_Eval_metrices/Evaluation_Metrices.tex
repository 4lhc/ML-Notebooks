...
\begin{document}
    ...
    \hypertarget{coding-assignment-5}{%
\section{Coding Assignment 5}\label{coding-assignment-5}}
...
Ref: \href{https://www.youtube.com/watch?v=wpQiEHYkBys}{Machine Learning
...
The autoreload extension is already loaded. To reload it, use:
  \%reload\_ext autoreload
    ...
    \hypertarget{loading-dataset-selecting-features}{%
\subsection{5.0. Loading dataset \& Selecting
features}\label{loading-dataset-selecting-features}}

Previously, it was observed that the four features,
\texttt{plasma\_glucose,\ serum\_insulin,\ bmi,\ diab\_pedigree} alone
gave the best accuracy. We will work with these four features as our
primary feature set in this notebook.
...
(768, 9)
    ...
    \hypertarget{checking-the-extent-of-class-imbalance}{%
\paragraph{Checking the extent of class
imbalance}\label{checking-the-extent-of-class-imbalance}}

We note that there is slight inbalance
\texttt{\textasciitilde{}\ 35:65}, which is not terribly bad. However,
this dataset is from a population that has a high prelavance of type 2
diabetes (US-Pima Indians). For other populations the class inbalance
can be \texttt{\textasciitilde{}\ 10:90} (taking into account the global
diabetes occurance).

Expected dummy score \textasciitilde{} 0.65
...
encountered in double\_scalars
  mcc = cov\_ytyp / np.sqrt(cov\_ytyt * cov\_ypyp)
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{Evaluation_Metrices_files/Evaluation_Metrices_7_1.png}
    \end{center}
    { \hspace*{\fill} \\}
    
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{Evaluation_Metrices_files/Evaluation_Metrices_7_2.png}
    \end{center}
    { \hspace*{\fill} \\}
...
\hypertarget{flipping-labels}{%
\paragraph{Flipping labels}\label{flipping-labels}}

It can be observed that every metrices except \texttt{MCC} and
\texttt{AUROC} changes. \texttt{ROC} plot is mirrored along the line
\texttt{y=1-x}, but \texttt{AUROC} remains unchanged
...
encountered in double\_scalars
  mcc = cov\_ytyp / np.sqrt(cov\_ytyt * cov\_ypyp)
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{Evaluation_Metrices_files/Evaluation_Metrices_10_1.png}
    \end{center}
    { \hspace*{\fill} \\}
    
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{Evaluation_Metrices_files/Evaluation_Metrices_10_2.png}
    \end{center}
    { \hspace*{\fill} \\}
