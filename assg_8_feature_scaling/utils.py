...
def plot_table(eval_metrics_list, ax, rowlabels, collabels):
    ax.set_axis_off()
    table = ax.table(
      cellText = eval_metrics_list,
      rowLabels = rowlabels,
      colLabels = collabels,
      rowColours =["skyblue"] * len(rowlabels),
...
def plot_confusion(cf_matrices, titles):
    ...
    """
    plt_cols = len(cf_matrices)
    fig, axes = plt.subplots(1, plt_cols, sharex=True, sharey=True,figsize=(10,2))

    for i, ax in enumerate(axes.flat):
        sns.heatmap(cf_matrices[i], ax=ax,cbar=i==(plt_cols-1), 
